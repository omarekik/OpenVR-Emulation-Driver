...
{
    static const std::unordered_map<std::string, int> kVKTable = {
        { "VK_SPACE",   VK_SPACE   },
        { "VK_LEFT",    VK_LEFT    },
        { "VK_RIGHT",   VK_RIGHT   },
        { "VK_UP",      VK_UP      },
    ...
    auto it = kVKTable.find(value);
    if (it != kVKTable.end()) return it->second;

    try {
        std::size_t pos = 0;
        int v = static_cast<int>(std::stoul(value, &pos, 0)); // base 0: auto-detects 0x
        return v;
    } catch (...) {}
    return 0;
}
...
IniMap ParseIniFile(const std::string& path)
{
    ...
    while (std::getline(file, line)) {
        ...
        Trim(key);
        Trim(val);
        if (!key.empty()) result[section][key] = val;
    }
    return result;
}
...
InputConfig InputConfig::LoadFromFile(const std::string& path)
{
    ...
    if (ini.empty()) return cfg; // file absent or empty – use struct defaults
    ...
    if (!rc("key_a").empty())          cfg.right_controller.key_a          = ParseKeyValue(rc("key_a"));
    if (!rc("key_b").empty())          cfg.right_controller.key_b          = ParseKeyValue(rc("key_b"));
    if (!rc("btn_a").empty())          cfg.right_controller.btn_a          = ParseXInputCombo(rc("btn_a"));
    if (!rc("btn_b").empty())          cfg.right_controller.btn_b          = ParseXInputCombo(rc("btn_b"));
    if (!rc("btn_grip").empty())       cfg.right_controller.btn_grip       = ParseXInputCombo(rc("btn_grip"));
    if (!rc("btn_system").empty())     cfg.right_controller.btn_system     = ParseXInputCombo(rc("btn_system"));
    if (!rc("btn_joystick_click").empty()) cfg.right_controller.btn_joystick_click = ParseXInputCombo(rc("btn_joystick_click"));
    if (!rc("trigger_click_threshold").empty())
        cfg.right_controller.trigger_click_threshold = ParseFloat(rc("trigger_click_threshold"), cfg.right_controller.trigger_click_threshold);
    if (!rc("pose_move_speed").empty())
        cfg.right_controller.pose_move_speed = ParseFloat(rc("pose_move_speed"), cfg.right_controller.pose_move_speed);
